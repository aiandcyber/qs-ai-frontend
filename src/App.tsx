...
import { API_BASE, AUTH_CONFIGURED } from './config'
...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'capture', label: 'nav_capture', icon: 'camera' },
  ...
  },
  {
    ...
    items: [
      { id: 'ipc', label: 'nav_ipc', icon: 'stamp' },
...
function NavIcon({ name }: { name: string }) {
  ...
  switch (name) {
    ...
    case 'stamp':
      return (
        <svg {...common}>
          <path d="M9 9a3 3 0 1 1 6 0c0 1.5-1 2-1 3v1h-4v-1c0-1-1-1.5-1-3z" />
          <path d="M5 20h14M6 17h12v3H6z" />
        </svg>
      )
    default:
      return (
...
export default function App() {
  ...
  const [captureItemNo, setCaptureItemNo] = useState('')
  const [ipcDraft, setIpcDraft] = useState<IpcDraft | null>(null)
  const [agentWidth, setAgentWidth] = useState<number>(() => {
    const v = Number(localStorage.getItem('qs-agent-width'))
    return v >= 320 ? v : 380
  })

  useEffect(() => {
    localStorage.setItem('qs-agent-width', String(agentWidth))
  }, [agentWidth])

  function startAgentResize(e: ReactMouseEvent) {
    e.preventDefault()
  ...
  useEffect(() => {
    getCaptureSources().then(setCaptureSources).catch(() => setCaptureSources([]))
  }, [])

  useEffect(() => {
    if (!projectId) return
    setIpcDraft(null)
    listCaptures(projectId).then(setCaptures).catch(() => setCaptures([]))
  }, [projectId])

  async function onAddCapture(source: CaptureSource) {
    setError('')
    try {
      await addCapture(projectId, { source, item_no: captureItemNo })
      setCaptures(await listCaptures(projectId))
    } catch (err) {
      setError(String(err))
    }
  }

  async function onGenerateIpc() {
    setLoading(true)
    setError('')
    try {
      setIpcDraft(await generateIpc(projectId))
    } catch (err) {
      setError(String(err))
    } finally {
      setLoading(false)
  ...
  return (
    <div
      className={`shell ui-modern ${agentCollapsed ? 'agent-collapsed' : ''}`}
      style={
        agentCollapsed
          ? undefined
          : { gridTemplateColumns: `272px minmax(0, 1fr) ${agentWidth}px` }
      }
    >
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          {page === 'capture' && (
            <div className="stack">
              <Panel title={t(locale, 'capture_title')}>
                <p className="muted">{t(locale, 'capture_hint')}</p>
                <label className="field-inline" style={{ marginBottom: 12 }}>
                  {t(locale, 'capture_item')}
                  <input
                    type="text"
                    value={captureItemNo}
                    placeholder="e.g. A.3.1"
                    onChange={(e) => setCaptureItemNo(e.target.value)}
                  />
                </label>
                <div className="module-grid">
                  {captureSources.map((s, i) => (
                    <div key={s.id} className="module-card static">
                      <strong>{i + 1}. {s.label}</strong>
                      <span className="muted">{s.detail}</span>
                      <button type="button" onClick={() => onAddCapture(s.id)}>
                        {t(locale, 'capture_add')}
                      </button>
                    </div>
                  ))}
                </div>
              </Panel>
              <Panel title={`${t(locale, 'capture_list')} (${captures.length})`}>
                {captures.length === 0 && <div className="empty">{t(locale, 'capture_none')}</div>}
                {captures.length > 0 && (
                  <table>
                    <thead>
                      <tr>
                        <th>ID</th>
                        <th>{t(locale, 'capture_source')}</th>
                        <th>{t(locale, 'capture_item')}</th>
                        <th>{t(locale, 'capture_desc')}</th>
                        <th>{t(locale, 'capture_percent')}</th>
                        <th>Time</th>
                      </tr>
                    </thead>
                    <tbody>
                      {captures.map((c) => (
                        <tr key={c.id}>
                          ...
                          <td>{c.item_no || '—'}</td>
                          <td>{c.description}</td>
                          <td className="mono">{c.percent_complete != null ? `${c.percent_complete}%` : '—'}</td>
          ...
          {page === 'ipc' && (
            <Panel title={t(locale, 'ipc_title')}>
              <p className="muted">{t(locale, 'ipc_hint')}</p>
              <div className="toolbar-row">
                <button type="button" onClick={onGenerateIpc} disabled={loading}>
                  {loading ? t(locale, 'analyzing') : t(locale, 'ipc_generate')}
                </button>
              </div>
              {ipcDraft && (
                <div className="pre" style={{ marginTop: 12 }}>{ipcDraft.ipc_text}</div>
              )}
            </Panel>
          )}
